...
with st.expander("Upload files & run extraction", expanded=(st.session_state.classified is None)):
    ...
    with col1:
        object_name = st.text_input("Salesforce Object", value="Account", key="object_name_input")
...
if st.session_state.classified:
    ...
    for i, e in enumerate(classified):
        ...
        # Visual priority flag -- makes rows needing attention immediately
        # scannable, instead of buried among auto-buildable ones.
        if action_code == "auto_generate":
            flag = "\u2705"  # green check -- nothing to do
        elif action_code == "flag_manual_review":
            flag = "\U0001F7E1"  # yellow -- quick confirmation needed
        else:
            flag = "\U0001F534"  # red -- needs a real decision (rebuild/retire/decision_needed/no_registry_entry)
    ...
    # Default sort: rows needing attention (red, then yellow) surface
    # first, auto-buildable (green) rows sink to the bottom -- so a
    # reviewer's attention naturally lands on what actually needs a
    # decision, not scattered throughout a long list.
    ...
    # Plain-English legend -- so a reviewer never has to ask what these
    # terms mean. Kept short and business-facing on purpose: this explains
    # WHAT to do, not the internal mechanics of how the tool decided.
    red_count = (df["\u26a0"] == "\U0001F534").sum()
    yellow_count = (df["\u26a0"] == "\U0001F7E1").sum()
    green_count = (df["\u26a0"] == "\u2705").sum()
    st.info(
        f"**How to read this table:** \u2705 **{green_count} will be built automatically** \u2014 "
        f"nothing to do. \U0001F7E1 **{yellow_count} need a quick confirmation** \u2014 the answer is "
        f"proposed, just needs a yes. \U0001F534 **{red_count} need your decision** \u2014 rows are "
        f"sorted so these appear first.\n\n"
        f"**What the terms mean:**\n"
        f"- **Direct** \u2014 a standard field that's identical on both systems. Nothing was decided, it just carries over.\n"
        f"- **Map** \u2014 a specific field where we've confirmed exactly what it becomes (same name or different).\n"
        f"- **Rebuild** \u2014 this should still exist, but the old way it worked can't carry over directly \u2014 needs custom development.\n"
        f"- **Retire** \u2014 this might not be needed anymore \u2014 flagged for you to confirm: keep it, or drop it?\n"
        f"- **Decision Needed** \u2014 genuinely unclear, needs your judgment before we can proceed."
    )
...
if st.session_state.approved_df is not None:
    ...
    #
    # IMPORTANT: must look up by the preserved 'idx' column, NOT by row
    # position. The table is sorted (red/needs-attention rows first,
    # green/auto-build rows last) for reviewer usability, so row position
    # no longer matches st.session_state.classified's original order.
