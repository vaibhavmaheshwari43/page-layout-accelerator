...
def _apply_layout_dropdown_selection():
    """Same safe pattern as _select_candidate_callback above -- must be a
    callback, not inline code, since the Layout Name text input has
    already rendered earlier in the script by the time this runs."""
...
with st.expander("Upload files & run extraction", expanded=(st.session_state.classified is None)):
    ...
    with col2:
        ...
        if input_mode == "Upload file":
            ...
            # Layout dropdown/checklist -- lets a reviewer pick a real
            # layout instead of typing/guessing its exact name.
            ...
            if load_layouts_clicked:
                ...
                with st.spinner(f"Looking up layouts for '{object_name}'..."):
                    names = list_layout_names_for_object(sf_exe, object_name, veeva_org_alias, veeva_sfdx_root)
