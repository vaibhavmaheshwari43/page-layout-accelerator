...
if st.session_state.classified:
    ...
    # "AI second opinion" feature that calls Claude directly to sanity-check
    # flagged rows. Using real '#' comments here, not a triple-quoted
    # """...""" string -- that approach is fragile: it only avoids a syntax
    # error if the indentation on EVERY line, including whatever comes right
    # after the closing """, lines up exactly with what Python expects at
    # that point in the file. A '#' comment has no such requirement -- it's
    # skipped by the parser entirely, regardless of indentation, so it can
    # never cause this kind of "unexpected indent" error.
    #
    ...
    #             df.at[i, "Status (system)"] = "auto_generate"
    #             applied_count += 1
    #     if applied_count:
