    ...
    for layout in all_layouts:
        ...
        if prefix_matches and name_similarity >= 0.5:
            candidates.append({
                ...
                # 'id' is already returned by 'sf org list metadata' --
                # reusing it means we don't need a separate lookup query
                # per candidate later, which was the main source of the
                # slowdown.
                "id": layout.get("id"),
            })
