...
with st.expander("Upload files & run extraction", expanded=(st.session_state.classified is None)):
    ...
    with col1:
        ...
        # Org alias is now always visible here (not just inside Fetch
        # mode), since the "Load layouts" lookup needs it regardless of
        # whether the layout itself ends up being uploaded or fetched --
        # this also means Fetch mode below reuses this SAME alias instead
        # of asking for it a second time.
        veeva_org_alias = st.text_input(
            "Veeva org alias (already authenticated via 'sf org login web')",
            value="veevaSource")

        # Layout dropdown/checklist -- always available right under the
        # object name, independent of Upload vs Fetch mode, so a person
        # can look up real layout names for reference even if they plan
        # to upload a file rather than fetch one directly.
    ...
    with col2:
        ...
        if input_mode == "Upload file":
            ...
            with st.expander("Advanced (only if the project folder isn't auto-detected correctly)"):
                veeva_sfdx_root = st.text_input("SFDX project root path", value=AUTO_SFDX_ROOT, key="veeva_fetch_root")
            fetch_clicked = st.button("Fetch layout from Veeva org")

        # Mapping Registry upload moved OUT of this section per request --
        # it conceptually belongs to Review & Approve (that's what it's
        # actually used for), not to fetching/extracting the layout
        # itself. It still has to be provided before "Run Extraction +
        # Classification" below, since that button does both extraction
        # AND classification together in one action -- but it's now in
        # its own clearly separate area, not grouped under "Extract".

        # Optional target-org describe upload -- commented out per
        # request, not deleted, so it can be re-enabled later without
...
if st.session_state.generated:
    ...
    with st.expander("Stage, pre-flight check, deploy, and verify", expanded=False):
        ...
        with col1:
            ...
            with st.expander("Advanced (only if the project folder isn't auto-detected correctly)"):
                sfdx_root = st.text_input("SFDX project root path", value=AUTO_SFDX_ROOT)
        with col2:
