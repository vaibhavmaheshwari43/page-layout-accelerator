...
def resolve_table_enum_ids(sf_exe, table_enum_values, org_alias, sfdx_root):
    """TableEnumOrId is a real Salesforce quirk: for STANDARD objects it
    returns a readable name (e.g. 'Account'), but for CUSTOM objects it
    returns the object's raw internal Id instead (e.g.
    '01I5w000002M0OiEAK') -- confirmed by seeing this exact behavior on a
    real org. That raw Id is technically valid/unique, but meaningless to
    a person reading the candidate list. This resolves any Id-shaped
    values back to their real, readable API name via EntityDefinition.

    Detection: real object API names contain letters and usually
    underscores (Account, Address_vod__c); Salesforce Ids are exactly 15
    or 18 characters, purely alphanumeric, never containing an
    underscore -- a reliable, simple way to tell them apart.
    """
    def looks_like_id(value):
        return len(value) in (15, 18) and value.isalnum() and "_" not in value

    ids_to_resolve = [v for v in table_enum_values if looks_like_id(v)]
    if not ids_to_resolve:
        return {}

    # Salesforce Ids exist in both a 15-char (case-sensitive) and 18-char
    # (case-insensitive) form. TableEnumOrId and EntityDefinition.DurableId
    # may not use the same length convention -- query for BOTH forms of
    # each Id, so a length mismatch alone doesn't cause a silent miss.
    all_id_variants = set()
    for i in ids_to_resolve:
        all_id_variants.add(i)
        if len(i) == 18:
            all_id_variants.add(i[:15])

    id_list = ", ".join(f"'{i}'" for i in all_id_variants)
    query = f"SELECT DurableId, QualifiedApiName FROM EntityDefinition WHERE DurableId IN ({id_list})"
    cmd = [sf_exe, "data", "query", "--use-tooling-api",
           "--query", query, "--target-org", org_alias, "--json"]
    try:
        result = subprocess.run(cmd, cwd=sfdx_root, capture_output=True, text=True,
        ...
        if result.returncode != 0:
            return {}
        data = json.loads(result.stdout)
        # Map DurableId (whichever length matched) -> name, then also map
        # each ORIGINAL requested Id (before any truncation) to that same
        # name, so callers can look up using the Id they actually have.
        raw_map = {r["DurableId"]: r["QualifiedApiName"]
                  for r in data.get("result", {}).get("records", []) if r.get("QualifiedApiName")}
        final_map = {}
        for original_id in ids_to_resolve:
            if original_id in raw_map:
                final_map[original_id] = raw_map[original_id]
            elif original_id[:15] in raw_map:
                final_map[original_id] = raw_map[original_id[:15]]
        return final_map
    except Exception:
        return {}
...
def search_layouts_by_name(sf_exe, layout_name, org_alias, sfdx_root):
    ...
    try:
        ...
        # Resolve any raw object Ids (from custom objects) back to real,
        # readable API names before building the display name.
        raw_table_enums = [r["TableEnumOrId"] for r in records if r.get("TableEnumOrId")]
        id_to_name = resolve_table_enum_ids(sf_exe, raw_table_enums, org_alias, sfdx_root)
        return [
...
def list_layouts_scoped(sf_exe, object_name, org_alias, sfdx_root):
    ...
    try:
        ...
        # to know or care which listing method was used. Also resolve any
        # raw object Ids to real readable API names -- same fix as above.
        raw_table_enums = [r["TableEnumOrId"] for r in records if r.get("TableEnumOrId")]
        id_to_name = resolve_table_enum_ids(sf_exe, raw_table_enums, org_alias, sfdx_root)
        reshaped = [
            {"fullName": f"{id_to_name.get(r['TableEnumOrId'], r['TableEnumOrId'])}-{r['Name']}", "id": r["Id"]}
...
def find_candidate_layouts(all_layouts, object_name, layout_name, sf_exe=None, org_alias=None, sfdx_root=None):
    ...
    for layout in all_layouts:
        ...
        # Note: candidates arriving here already came from a real,
        # server-side name search (search_layouts_by_name) -- Salesforce
        # itself already confirmed these are relevant matches. A strict
        # local similarity cutoff on top of that was silently discarding
        # genuine candidates whenever the typed name differed more than
        # trivially (confirmed bug: this caused only 1 of several real
        # matches to ever be shown). Lowered significantly -- the top-5 cap
        # below is what keeps the list relevant, not this threshold.
