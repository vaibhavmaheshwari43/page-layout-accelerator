...
if st.session_state.generated:
    ...
    with tab1:
        ...
        with panel_col1:
            ...
            if veeva_screenshot:
                st.image(veeva_screenshot, use_container_width=True)
        ...
        with panel_col2:
            ...
            if current_screenshot:
                st.image(current_screenshot, use_container_width=True)
                st.caption("Real screenshot")
        ...
        with panel_col3:
            ...
            if after_screenshot:
                st.image(after_screenshot, use_container_width=True)
            ...
            if not os.path.isfile(sfdx_project_json):
                ...
                try:
                    with st.spinner("Running dry-run deploy..."):
                        result = subprocess.run(cmd, cwd=sfdx_root, capture_output=True, text=True, timeout=120)
        ...
        if real_deploy_clicked:
            ...
            try:
                with st.spinner("Deploying for real..."):
                    result = subprocess.run(cmd, cwd=sfdx_root, capture_output=True, text=True, timeout=180)
                st.code(result.stdout or "(no stdout)")
                if result.returncode == 0:
        ...
        if st.button("Fetch deployed layout & verify", disabled=not st.session_state.get("deploy_succeeded", False)):
            ...
            try:
                with st.spinner("Retrieving deployed layout from the org..."):
                    result = subprocess.run(retrieve_cmd, cwd=sfdx_root, capture_output=True, text=True, timeout=120)
                st.code(result.stdout or "(no stdout)")
                if result.returncode != 0:
