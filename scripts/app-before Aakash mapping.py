...
def _select_candidate_callback(full_name, org_alias, sfdx_root):
    ...
    already rendered in the same run, which is exactly what happened here
    the first time (StreamlitAPIException). Callbacks run before the
    widgets re-render on the next pass, so this is the correct pattern."""
    ...
    fetch_result = fetch_veeva_layout(prefix, rest, org_alias, sfdx_root)
